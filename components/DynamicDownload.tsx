...
const DownloadButton = ({ type }: { type: OSType }) => {
  ...
  return (
    <div className="mt-8 space-x-2 md:space-x-4 flex justify-center w-full">
      ...
      >
        <svg className="text-white h-6 w-6 mr-2" fill="white" viewBox={type.svgViewBox} xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
          <title>{type.name}</title>
          <path d={type.svgPath} />
        </svg>
        <span>
          {t("download.for")} {t(`download.${type.locale}`)}
        </span>
      </Link>
    </div>
  );
